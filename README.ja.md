...
- **Claude Code HUD** — Claude Code の hooks を通じて、エージェントの今の状態をリアルタイム表示:
  思考中 / 実行中のツール / 確認待ち / 完了。
- **AI コンパニオンカード** — アイドル時にローカル/LAN の LLM が短く温かい、時に気の利いた一言を生成。
...
flowchart LR
    ...
    BR -- "prompt" --> LLM
    LLM -- "コンパニオンカード" --> BR
    BR -- "ESPHome ネイティブ API(暗号化)<br/>show_card · set_metrics" --> DEV
    DEV -- "ボタン押下 (state)" --> BR
```

LLM はすべてデバイス外で動作する(ボードの性能では動かせない)。デバイスは表示・タッチ・ボタン報告のみ。
bridge は **意味だけを push し、フレームは push しない**:言語非依存の `status` キー
(`run`/`think`/`wait`/`done`/`ready`/`online`)がデバイス側の色/アニメーションを駆動し、
ローカライズされた `mood`/`title`/`body` はテキストとして表示される。
...
- ステータス連動のアクセントカラーを mood ラベルだけでなく目/アニメーションにも適用。
